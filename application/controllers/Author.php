...
class Author extends MY_Controller
{
	...
	public function edit($tag)
	{
		...
		if ($numeric == true && $notNumeric == false) {
			if (($this->author_model->get_author_byId($tag))) {
				...
				$this->session->set_userdata('oldImageFile', $data['author'][0]->image);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules(
			'id',
			'id',
			'trim|required|numeric|min_length[1]|max_length[20]',
			array(
				'required'      => 'You have not provided %s.',
			)
		);
		...
		$this->form_validation->set_rules(
			'first_name',
			'FirstName',
			'trim|required|min_length[5]|max_length[22]',
			array(
				'required'      => 'You have not provided %s.',
				'is_unique'     => 'This %s already exists.'
			)
		...
		$this->form_validation->set_rules(
			'email',
			'email',
			'trim|required|valid_email|min_length[5]|max_length[40]',
			array(
				'required'      => 'You have not provided %s.',
				'is_unique'     => 'This %s already exists.'
			)
		...
		$this->form_validation->set_rules(
			'country',
			'country',
			'trim|required|alpha|min_length[3]|max_length[25]',
			array(
				'required'      => 'You have not provided %s.',
			)
		);
		...
		$this->form_validation->set_rules(
			'about',
			'about',
			'trim|required|min_length[50]|max_length[450]',
			array(
				'required'      => 'You have not provided %s.',
			)
		);
		...
		$this->form_validation->set_rules(
			'age',
			'age',
			'trim|required|numeric|is_natural|min_length[2]|max_length[5]',
			array(
				'required'      => 'You have not provided %s.',
			)
		);
		...
		$this->form_validation->set_rules(
			'university',
			'university',
			'trim|required|alpha|min_length[3]|max_length[35]',
			array(
				'required'      => 'You have not provided %s.',
			)
		);
		...
		$this->form_validation->set_rules(
			'publisher',
			'publisher',
			'trim|required|alpha|min_length[5]|max_length[65]',
			array(
				'required'      => 'You have not provided %s.',
			)
		);
		...
		if ($this->form_validation->run() === FALSE) {
			$this->loadhead();
			$this->load->view('dashboard/author/edit');
			$this->loaddown();
		} else {
			// validate upload file
			$this->form_validation->set_rules('image', 'File', 'trim|xss_clean');
			...
			if (!$this->upload->do_upload('image')) {
				...
				// $this->load->view('upload_form', $error);
				$this->loadhead();
				$this->load->view('dashboard/author/edit', $error);
				$this->loaddown();
				return;
			} else {
				//if file does not exist upload it
			...
			if ($this->author_model->update_author($this->imagePath)) {
				//update success 

				//remove old image uploaded
				//get old image name by session seted from edit method
				unlink( FCPATH . "uploads/" .$this->session->oldImageFile);
				//remove image session data
				$this->session->unset_userdata('oldImageFile');
				...
				//show error and update failure

				//remove current uploaded image file
				echo unlink( FCPATH . "uploads/" . $this->imagePath );

				$this->session->set_flashdata('error', 'author does not updated! :|');
				//redirect to create author
				$this->loadhead();
				$this->load->view('dashboard/author/create');
				$this->loaddown();
